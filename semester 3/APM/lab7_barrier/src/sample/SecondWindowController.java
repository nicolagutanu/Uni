...
public class SecondWindowController {
    ...
    @FXML
    private TableColumn<BarrierTableView,String> barrierTable_listColumn;
    @FXML
    ...
    public void initialize() {
        ...
        barrierTable_IndexColumn.setCellValueFactory(new PropertyValueFactory<BarrierTableView,Integer>("index"));
        barrierTable_valueColumn.setCellValueFactory(new PropertyValueFactory<BarrierTableView,Integer>("value"));
        barrierTable_listColumn.setCellValueFactory(new PropertyValueFactory<BarrierTableView,String>("list"));
    ...
    //populate barrier table
    public void populateBarrierTable() {
        if (!controller.getRepo().getAll().isEmpty()) {
            IBarrier<Integer, BarrierPair> barrierTable=controller.getRepo().getAll().get(0).getBarrierTable();
            barrierTable_tableview.getItems().clear();
            for(Map.Entry<Integer,BarrierPair> e:barrierTable.getContent().entrySet())
                barrierTable_tableview.getItems().add(new BarrierTableView(e.getKey(),barrierTable.getValue(e.getKey()).getKey(),barrierTable.getValue(e.getKey()).toString()));
        }
    }
}
