...
public class ForkStmt implements IStatement {
    ...
    public PrgState execute(PrgState state) throws StmtException, ExpException, AdtException {
        ...
        return new PrgState(newStack, newSymTbl, state.getHeap(), state.getList(), state.getFileTable(), state.getLockTable(), state.getBarrierTable(), state.getLatchTable(),state.getSemaphoreTable(),stmt);
    }
