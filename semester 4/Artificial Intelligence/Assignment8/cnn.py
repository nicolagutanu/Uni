...
import numpy as np
from createDB import ImageClassifierDataset
...
image_list = []
image_classes = []
folders = ["test/people", "test/other"]
for folder in folders:
    for file in os.listdir(folder):
        img_data = Image.open(folder + "/" + file).convert('RGB')
        img_data.thumbnail((200, 200))
        image_list.append(img_data)
        if folder == 'test/people':
            image_classes.append("person")
        else:
            image_classes.append("other")
test_set = ImageClassifierDataset(image_list, image_classes)
...
def train(num_epochs):
    ...
    for epoch in range(num_epochs):
        ...
        train_accurracy.append(train_acc)
        train_losses.append(train_loss)
    return train_accurracy, train_loss


if __name__ == "__main__":
    train_acc, train_loss = train(200)
    plt.plot(train_acc, train_loss)
