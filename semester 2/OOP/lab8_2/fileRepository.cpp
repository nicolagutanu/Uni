...
vector<string> FileRepository::convertLine(std::string Plant, char Delimiter)
{
	stringstream stringPlant(Plant);
	string fieldOfPlant;
	vector<string> plant;
	while (getline(stringPlant, fieldOfPlant, Delimiter))
		plant.push_back(fieldOfPlant);
	return plant;
}
...
/* ASISSTANT */
void FileRepository::addPlantDisplacedList(const Plant& plantToDisplace)
{
	this->assistantList.push_back(plantToDisplace);
}
...
int FileRepository::plantExistsInDisplacedList(const Plant& plant)
{
	vector<Plant>::iterator it = find(this->assistantList.begin(), this->assistantList.end(), plant);
	if (it != this->assistantList.end())
	{
		return 1;
	}
	return 0;
...
vector<Plant> FileRepository::getPlantsToBeDisplaced()
{
